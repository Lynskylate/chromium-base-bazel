...
void IsTimerEnabled(bool *isEnabled) {
  ...
  *isEnabled = (current_timer.it_value.tv_sec != 0 ||
          current_timer.it_value.tv_usec != 0);
}
...
class ProfileHandlerUnitTest : public ::testing::Test {
 protected:
  ...
  void VerifyRegistration(const int& tick_counter) {
    ...
    ASSERT_GT(GetCallbackCount(), 0);
    // Check that the profile timer is enabled.
    bool timerEnabled = false;
    ASSERT_NO_FATAL_FAILURE(IsTimerEnabled(&timerEnabled));
    ASSERT_EQ(FLAGS_test_profiler_enabled, linux_per_thread_timers_mode_ || timerEnabled);
    ...
    if (FLAGS_test_profiler_enabled) {
      ASSERT_GT(new_tick_count, old_tick_count);
      ASSERT_GT(interrupts_after, interrupts_before);
    } else {
      ASSERT_EQ(new_tick_count, old_tick_count);
      ASSERT_EQ(interrupts_after, interrupts_before);
    }
  }
  ...
  void VerifyUnregistration(const int& tick_counter) {
    ...
    Delay(kSleepInterval);
    int new_tick_count = tick_counter;
    ASSERT_EQ(old_tick_count, new_tick_count);
    // If no callbacks, timer should be disabled.
    if (GetCallbackCount() == 0) {
      bool timerEnabled = false;
      ASSERT_NO_FATAL_FAILURE(IsTimerEnabled(&timerEnabled));
      ASSERT_FALSE(timerEnabled);
    }
  }
  ...
  void VerifyDisabled() {
    ...
    Delay(kSleepInterval);
    uint64 interrupts_after = GetInterruptCount();
    ASSERT_EQ(interrupts_before, interrupts_after);
  }
...
TEST_F(ProfileHandlerUnitTest, RegisterUnregisterCallback) {
  int tick_count = 0;
...
TEST_F(ProfileHandlerUnitTest, MultipleCallbacks) {
...
TEST_F(ProfileHandlerUnitTest, Reset) {
...
TEST_F(ProfileHandlerUnitTest, RegisterCallbackBeforeThread) {
  // Stop the worker.
  StopWorker();
