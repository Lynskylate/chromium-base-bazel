...
namespace base {
    ...
    string16 DateIntervalFormat(const Time& begin_time,
                                ...
                                DateFormat format) {
      UErrorCode status = U_ZERO_ERROR;
      const char *date_format = DateFormatToString(format);
      // Not sure what happened with the dateinterval c string stuff here?
      std::unique_ptr<icu::DateIntervalFormat> formatter(
              icu::DateIntervalFormat::createInstance(
                      icu::UnicodeString(date_format, strlen(date_format)),
                      status));
