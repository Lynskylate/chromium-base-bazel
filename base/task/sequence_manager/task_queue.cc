...
TaskQueue::~TaskQueue() {
  ...
  // No ideawhy this works, but it seems to resolve what looks to be something like a race condition
  // when built with bazel/g++, probably compiler differences
  internal::SequenceManagerImpl *manager = impl_->sequence_manager();
  ...
  manager->ShutdownTaskQueueGracefully(TakeTaskQueueImpl());
}
